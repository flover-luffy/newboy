...
public class CommandOperator extends AsyncWebHandlerBase {
    ...
    public Message executePublic(String[] args, Group g, long senderID) {
        ...
        if (fullCommand.startsWith("/zj") || fullCommand.startsWith("总结")) {
            try {
                // 创建DailySummaryCommand实例
                net.luffy.command.DailySummaryCommand handler = new net.luffy.command.DailySummaryCommand();
                ...
                if (parts.length >= 2) {
                    ...
                    // 处理英文命令
                    if (fullCommand.startsWith("/zj")) {
                        switch (subCommand) {
                            case "help":
                                // 直接发送帮助信息
                                g.sendMessage("📋 每日总结系统帮助\n\n" +
                                    "🔹 /zj help - 显示帮助信息\n" +
                                    "🔹 /zj status - 查看系统状态\n" +
                                    "🔹 /zj today - 生成今日总结\n" +
                                    "🔹 /zj yesterday - 生成昨日总结\n" +
                                    "🔹 /zj generate YYYY-MM-DD - 生成指定日期总结\n" +
                                    "🔹 /zj test - 测试系统功能\n" +
                                    "🔹 /zj restart - 重启总结服务\n" +
                                    "🔹 /zj room <房间ID> [日期] - 生成指定房间总结\n" +
                                    "🔹 /zj rooms - 查看可用房间列表\n\n" +
                                    "📝 中文命令：\n" +
                                    "🔹 总结 帮助 - 显示帮助信息\n" +
                                    "🔹 总结 状态 - 查看系统状态\n" +
                                    "🔹 总结 今天 - 生成今日总结\n" +
                                    "🔹 总结 昨天 - 生成昨日总结\n" +
                                    "🔹 总结 生成 YYYY-MM-DD - 生成指定日期总结\n" +
                                    "🔹 总结 测试 - 测试系统功能\n" +
                                    "🔹 总结 重启 - 重启总结服务\n" +
                                    "🔹 总结 房间 <房间ID> [日期] - 生成指定房间总结\n" +
                                    "🔹 总结 房间列表 - 查看可用房间列表");
                                return null;
                            case "status":
                            case "状态":
                                // 调用状态命令处理方法
                                try {
                                    net.luffy.util.summary.DailySummaryIntegration summaryIntegration = 
                                        net.luffy.util.summary.DailySummaryIntegration.getInstance();
                                    String status = summaryIntegration.getSystemStatus();
                                    String overview = summaryIntegration.getTodayOverview();
                                    
                                    g.sendMessage("📊 每日总结系统状态\n\n" + status + "\n\n" + overview);
                                } catch (Exception e) {
                                    g.sendMessage("获取状态信息失败: " + e.getMessage());
                                }
                                return null;
                            case "test":
                            case "测试":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        ...
                                        g.sendMessage("❌ 测试图片生成失败，请检查系统状态");
                                    }
                                } catch (Exception e) {
                                    g.sendMessage("生成测试图片时发生错误: " + e.getMessage());
                                }
                                return null;
                            case "rooms":
                            case "房间列表":
                                ...
                                try {
                                    net.luffy.util.summary.DailySummaryIntegration summaryIntegration = 
                                        net.luffy.util.summary.DailySummaryIntegration.getInstance();
                                    java.util.List<String> activeRoomIds = summaryIntegration.getActiveRoomIds();
                                    
                                    if (activeRoomIds.isEmpty()) {
                                        g.sendMessage("📭 当前没有活跃的房间数据");
                                        return null;
                                    }
                                    ...
                                    message.append("\n💡 使用 /zj room <roomId> 生成指定房间的总结");
                                    
                                    g.sendMessage(message.toString());
                                } catch (Exception e) {
                                    g.sendMessage("获取活跃房间列表失败: " + e.getMessage());
                                }
                                return null;
                            case "today":
                            case "今天":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        // 发送成功消息并自动发送图片
                                        g.sendMessage("✅ 今日总结生成成功！");
                                        
                                        // 自动发送生成的图片
                                        try {
                                            net.luffy.util.sender.MessageSender messageSender = new net.luffy.util.sender.MessageSender();
                                            String imagePath = "data/daily_images/daily_content_summary_" + java.time.LocalDate.now() + ".png";
                                            messageSender.sendGroupLocalImage(String.valueOf(g.getId()), imagePath);
                                        } catch (Exception imageException) {
                                            g.sendMessage("图片发送失败: " + imageException.getMessage());
                                        }
                                    } else {
                                        g.sendMessage("❌ 今日总结生成失败，请检查日志或稍后重试");
                                    }
                                } catch (Exception e) {
                                    g.sendMessage("生成今日总结时发生错误: " + e.getMessage());
                                }
                                return null;
                            case "yesterday":
                            case "昨天":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        // 发送成功消息并自动发送图片
                                        g.sendMessage("✅ 昨日总结生成成功！");
                                        
                                        // 自动发送生成的图片
                                        try {
                                            net.luffy.util.sender.MessageSender messageSender = new net.luffy.util.sender.MessageSender();
                                            String imagePath = "data/daily_images/daily_content_summary_" + java.time.LocalDate.now().minusDays(1) + ".png";
                                            messageSender.sendGroupLocalImage(String.valueOf(g.getId()), imagePath);
                                        } catch (Exception imageException) {
                                            g.sendMessage("图片发送失败: " + imageException.getMessage());
                                        }
                                    } else {
                                        g.sendMessage("❌ 昨日总结生成失败，可能没有足够的数据或系统未初始化");
                                    }
                                } catch (Exception e) {
                                    g.sendMessage("生成昨日总结时发生错误: " + e.getMessage());
                                }
                                return null;
                            case "restart":
                            case "重启":
                                // 调用重启命令处理方法
                                try {
                                    g.sendMessage("🔄 正在重启每日总结系统...");
                                    net.luffy.util.summary.DailySummaryIntegration summaryIntegration = 
                                        net.luffy.util.summary.DailySummaryIntegration.getInstance();
                                    boolean success = summaryIntegration.restart();
                                    
                                    if (success) {
                                        g.sendMessage("✅ 每日总结系统重启成功！");
                                    } else {
                                        g.sendMessage("❌ 系统重启失败，请检查日志");
                                    }
                                } catch (Exception e) {
                                    g.sendMessage("重启系统时发生错误: " + e.getMessage());
                                }
                                return null;
                            default:
                                return new PlainText("❌ 未知命令: " + subCommand + "\n请使用 '/zj help' 查看可用命令");
                        }
                    }
                    // 处理中文命令
                    else if (fullCommand.startsWith("总结")) {
                        switch (subCommand) {
                            case "帮助":
                                // 直接发送帮助信息（中文版）
                                g.sendMessage("📋 每日总结系统帮助\n\n" +
                            ...
                            case "测试":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        g.sendMessage("✅ 测试图片生成成功！图片已保存到 data/daily_images/test_image.png");
                                    } else {
                                        g.sendMessage("❌ 测试图片生成失败，请检查系统状态");
                                    }
                                } catch (Exception e) {
                            ...
                            case "今天":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        g.sendMessage("✅ 今日总结生成成功！图片已保存到 data/daily_images/ 目录");
                                    } else {
                                        g.sendMessage("❌ 今日总结生成失败，请检查日志或稍后重试");
                                    }
                                } catch (Exception e) {
                            ...
                            case "昨天":
                                ...
                                try {
                                    ...
                                    if (success) {
                                        g.sendMessage("✅ 昨日总结生成成功！图片已保存到 data/daily_images/ 目录");
                                    } else {
                                        g.sendMessage("❌ 昨日总结生成失败，可能没有足够的数据或系统未初始化");
                                    }
                                } catch (Exception e) {
                    ...
                    // 没有子命令，显示帮助信息
                    return new PlainText("❌ 请指定子命令\n使用 '/zj help' 或 '总结 帮助' 查看可用命令");
                }
                
            } catch (Exception e) {
                return new PlainText("处理每日总结命令时发生错误: " + e.getMessage());
            }
        }
